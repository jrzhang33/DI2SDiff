...
class Unet1D_cond(nn.Module):
    def __init__(
        ...
        # style embeddings
        if context_using:
            self.classes_emb = nn.Linear(num_classes, dim)
    ...
    def forward_with_cond_scale(  
        ...
        **kwargs
        ...
        for i in range(len(args[-1])):
            null_logits[i] = null_logits[i] / len((args[-1][i]))
    ...
    def forward(self, x_input, time_input, classesbatch, combination_dict, cond_drop_prob=None):
...
class GaussianDiffusion1Dcond(nn.Module):
    ...
    def model_predictions(self, x, t, classes,  combination_dict, cond_scale = 6., rescaled_phi = 0.7, clip_x_start = False):
        model_output = self.model.forward_with_cond_scale(x, t, classes,  combination_dict, cond_scale = cond_scale, rescaled_phi = rescaled_phi)
    ...
    def p_mean_variance(self, x, t, classes,  combination_dict, cond_scale, rescaled_phi, clip_denoised = True):
        preds = self.model_predictions(x, t, classes, combination_dict, cond_scale, rescaled_phi)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, classes, combination_dict, cond_scale = 6., rescaled_phi = 0.7, clip_denoised = True, ):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((x.shape[0],), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x = x, t = batched_times, classes = classes,  combination_dict =  combination_dict, cond_scale = cond_scale, rescaled_phi = rescaled_phi, clip_denoised = clip_denoised)
        ####### 1.2
    ...
    @torch.no_grad()
    def p_sample_loop(self, classes, combination_dict, shape, cond_scale = 6., rescaled_phi = 0.7):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            img, x_start = self.p_sample(img, t, classes,  combination_dict, cond_scale, rescaled_phi)
    ...
    @torch.no_grad()
    def sample(self, classes, combination_dict, cond_scale = 6., rescaled_phi = 0.7):
        batch_size, seq_length, channels = classes.shape[0], self.seq_length, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(classes, combination_dict, (batch_size, channels, seq_length), cond_scale, rescaled_phi)
