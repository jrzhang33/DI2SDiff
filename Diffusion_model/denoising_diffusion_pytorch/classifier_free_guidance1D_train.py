...
class GaussianDiffusion1Dcond(nn.Module):
    ...
    @torch.no_grad()
    def extractor(self, x, t: int, classes, cond_scale = 6., rescaled_phi = 0.7, clip_denoised = True):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((x.shape[0],), t, device = x.device, dtype = torch.long)
        feature_v = self.model.forward_feature(x, batched_times, classes)
        return feature_v

        
    @torch.no_grad()
    def get_featuremap(self, x1, x2, classes, t = None, lam = 0.5):
        ...
        for i in tqdm(reversed(range(0, t)), desc = 'interpolation sample time step', total = t):
            ...
            if i == 10:
                feature_vector = self.extractor(img, 50, classes)
    ...
    @torch.no_grad()
    def interpolate_step(self, x1, x2, classes, t = None, lam = 0.5):
        ...
        for i in tqdm(reversed(range(0, t)), desc = 'interpolation sample time step', total = t):
            img, _ = self.p_sample(img, i, classes)
            if i == 10 or i == 15 or i == 5 or i == 20 or i ==3 or i == 8 or i == 30 or i == 25:
                imglist.append(img)
        imglist.append(img)
        return img,imglist
