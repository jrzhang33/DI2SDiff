...
def test_model(fea,y,d,model, test_loader, model_file=None, batch_size=128):
    ...
    with torch.no_grad():
        ...
        for iter_n in range(test_batch_num):
            x_ori, label_ori, auglabel_ori,domain_label = next(iter_test_ori)
            ...
            act_label_p, _,feature,feature_aug = model.test_predict(x_ori, x_aug)
            _, predict_act = torch.max(act_label_p.data, 1)
            correct_act += (predict_act == label_ori).sum()
            total_act += label_ori.size(0)
            fea.append(torch.cat([feature,feature_aug],dim=0))
            y.append(torch.cat([label_ori,label_aug],dim=0))
            d.append(torch.cat([domain_label - 1,torch.ones(x_aug.shape[0])*14],dim=0))




    act_acc_test = float(correct_act) * 100 / total_act
    return act_acc_test,fea,y,d
