...
def raw_to_aug(seed, root_path, save_path, dataset, aug_num=7, remain_data_rate=0.2):
    ...
    for i in range(len(data)):
        ...
        try:
            x_train_i, y_train_i, s_train_i = pick_data(
                data, 'train', src=i)
            x_val_i, y_val_i, s_val_i = pick_data(data, 'val', src=i)
            x_test_i, y_test_i, s_test_i = pick_data(data, 'test', src=i)
            y_train_i = np.concatenate((y_train_i, s_train_i), axis=1)
            y_val_i = np.concatenate((y_val_i, s_val_i), axis=1)
            y_test_i = np.concatenate((y_test_i, s_test_i), axis=1)
        except:
...
def augment(x, y, aug_num):
    ...
    for aug in range(aug_num):
        aug_all_y.append(np.ones((x.shape[0])) * y[0])
...
if __name__ == "__main__":
    ...
    raw_all, aug_all = raw_to_aug(seed=args.seed, root_path="/home/data/process/",
                                  save_path="/home/data/process/", dataset='uschad', aug_num=7, remain_data_rate=0.8)
